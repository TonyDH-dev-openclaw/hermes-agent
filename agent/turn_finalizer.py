...
def finalize_turn(
    ...
    *,
    ...
    #
    # NOTE: this fix has been silently reverted twice already by an
    # automated `git reset --hard origin/main` elsewhere in this
    # environment (see reflog around 2026-07-28) -- if you're reading this
    # comment in a diff that's about to remove the unconditional call
    # below, stop and check whether that's really intended before reverting.
    try:
        from hermes_cli.lifecycle import invoke_hook as _invoke_hook
        _invoke_hook(
            "post_llm_call",
